...
class StressTest:
    ...
    async def setup(self):
        ...
        print(f"用户名格式: {self.config.username_prefix}1 ~ {self.config.username_prefix}{self.config.num_users}")
        print(f"使用代理: {'是' if self.config.use_proxy else '否'}")
        print(f"并发批次: {self.config.concurrent_batch}")
        print("=" * 60)
    ...
    def save_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "total_time": self.test_end_time - self.test_start_time,
                "throughput": self.config.num_users / (self.test_end_time - self.test_start_time),
                "login_success": sum(1 for r in self.results if r.login_success),
                "register_success": sum(1 for r in self.results if r.register_success),
                "submit_success": sum(1 for r in self.results if r.submit_success),
            },
...
async def main():
    """主函数"""
    if len(sys.argv) > 1:
        config = TestConfig.from_json(sys.argv[1])
    else:
        config = TestConfig.from_json('stress_test_config.json')
