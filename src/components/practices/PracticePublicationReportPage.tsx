/**
 * 发布实例做题信息页面（教师端 / 管理员端共享）。
 * 按发布实例 ID 拉取做题报告（参与人数、提交数、学生排名、提交记录），支持查看代码与导出 CSV。
 */
...
export function PracticePublicationReportPage({ variant }: { variant: Variant }) {
  const navigate = useNavigate();
  const { publicationId } = useParams();
  const numericId = publicationId ? Number(publicationId) : undefined;

  const listPath = variant === 'teacher' ? '/teacher/practices' : adminPath('/practices');
  const getRequest = variant === 'teacher' ? teacherGet : adminGet;
  const codePath = (id: number) => variant === 'teacher'
    ? `/api/teacher/v1/submissions/${id}/code`
    : `/api/admin/v1/submissions/${id}/code`;
  ...
  useEffect(() => {
    if (numericId) void loadReport();
  }, [numericId]);

  async function loadReport() {
    setLoading(true);
    try {
      const result = await getRequest<PracticeReport>(`/api/admin/v1/practices/publications/${numericId}/report`);
      setReport(result);
    } catch (error) {
  ...
  function exportRankings() {
    ...
    downloadCsv(header + rows, `publication_${publicationId}_rankings.csv`);
  }
  ...
  function exportSubmissions() {
    ...
    downloadCsv(header + rows, `publication_${publicationId}_submissions.csv`);
  }
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card
        ...
        title="发布做题信息"
        loading={loading}
        extra={<Button icon={<IconLeft />} onClick={() => navigate(listPath)}>返回列表</Button>}
      >
        {report && (
          <Row gutter={16} style={{ marginBottom: 24 }}>
            ...
            <Col span={8}>
              <Statistic title="通过题数（最高）" value={report.rankings?.length ? Math.max(...report.rankings.map((r) => r.solved)) : 0} />
            </Col>
          </Row>
        )}
      </Card>
      ...
      {report && (
        <>
          <Card
            bordered={false}
            title="学生排名"
            extra={<Button icon={<IconDownload />} onClick={exportRankings}>导出排名</Button>}
          >
            <Table
              rowKey="userId"
          ...
          <Card
            bordered={false}
            title="提交记录"
            extra={<Button icon={<IconDownload />} onClick={exportSubmissions}>导出提交</Button>}
          >
            <Table
              rowKey="id"
